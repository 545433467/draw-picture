...
- `资源分组` 值为 `CCE_Deployment`（或服务类型标注为 CCE_Deployment）的资源，业务框改按 `resource_name` 前 3 段（`-` 分隔）划分：例如 `bj-prod-app-...` 与 `bj4-prod-app-...` 各自成为一个业务框；同一业务框内再按 `enterprise_project_id`（企业项目）细分为子容器，每个企业项目子容器独立执行“前 5 个 + 摘要”聚合。
