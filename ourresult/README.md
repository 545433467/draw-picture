...
- `资源分组` 值为 `CCE_Deployment`（或服务类型标注为 CCE_Deployment）的资源，业务框仍按 `所属业务` 字段划分；这些资源固定落在业务内的“第三层：计算、容器与应用服务层”容器中，服务框内先按 `resource_name` 前 3 段（`-` 分隔，可通过 `converter.py` 中 `CCE_BUSINESS_PREFIX_SEGMENTS` 常量改为前 4 段）细分为 CCE 分组容器（例如 `bj-prod-app-...` 与 `bj4-prod-app-...` 各自成为一个分组容器），分组容器内再按 `enterprise_project_id`（企业项目）细分为子容器，每个企业项目子容器独立执行“前 5 个 + 摘要”聚合。
- 未填写 `所属业务` 的 CCE_Deployment 节点，若存在“企业项目相同且 `resource_name` 前缀（前 3 段，可配置为前 4 段）相同”的业务，则退而求其次并入该业务分组（同前缀的候选业务有多个时，并入匹配 CCE 节点数最多的）；企业项目相同但前缀不同，或找不到匹配业务时，才放入以服务类型命名的虚拟业务框，并同样出现在计算层容器中。
