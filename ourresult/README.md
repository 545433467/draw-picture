...
- 所有资源均按 `资源分组` 聚合；点击 `...+n` 摘要节点或数量徽标后，右侧抽屉展示该分组全部资源明细，并提供逐行“编辑”按钮。
...
- **全量资源聚合**：普通资源按架构层、`resource_type`、资源分组和资源名非数字结构生成缩略节点（`分组名 + ...+N`）；CCE_Deployment 按原资源分组规则聚合。同一 `resource_type` 的所有缩略节点统一置于对应的大框（如 `ECS`）中；大框名称使用 `resource_type`，资源分组框名称使用 Excel 的 `资源分组` 值。
