...
- 资源按 `所属业务 + 所属层 + resource_type + 资源分组` 聚合，所有受支持服务类型均使用与原 CCE/ECS 相同的 `...+N` 摘要节点；同一资源分组的明细全部保留在右侧抽屉中。
...
- **全量资源聚合**：每个业务内按架构层、`resource_type` 和资源分组生成缩略节点（`分组名 + ...+N`），同一 `resource_type` 的所有缩略节点统一置于对应的大框（如 `ECS`）中；右上角带数量徽标，上游资源只需一条调用线指向该缩略节点。
