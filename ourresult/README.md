...
- **CCE / CCE_Deployment / ECS 聚合**：在现有“资源分组”基础上聚合——每个业务内，同一资源分组（如 `Web应用层`、`CCE_Deployment`、`CCE工作负载`、`K8s集群`）的 CCE/CCE_Deployment/ECS 资源合并为一个缩略节点（`分组名 + ...+N`），不同资源分组各自独立缩略，不会全部揉成一个；节点显示对应服务图标（保留原图、不变形），右上角带一个红色圆圈数字徽标表示数量；上游资源（如 ELB）只需一条调用线指向该缩略节点；点击缩略节点或数字徽标可在右侧抽屉查看全部折叠资源的明细表格。
