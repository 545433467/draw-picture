...
- `资源分组` 值为 `CCE_Deployment`（或服务类型标注为 CCE_Deployment）的资源，业务框仍按 `所属业务` 字段划分；这些资源固定落在业务内的“第三层：计算、容器与应用服务层”容器中，服务框内直接按 `enterprise_project_id`（企业项目）细分为一个层级的企业项目容器（框名即为企业项目字段），每个企业项目容器独立执行“前 5 个 + 摘要”聚合；不再使用 `resource_name` 划分多层嵌套框。
