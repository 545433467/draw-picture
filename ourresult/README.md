...
- 业务关键字过滤：只绘制 `所属业务` 包含 `chat`（不区分大小写）的业务及其全部节点，其他业务不显示；可通过 `converter.py` 中 `BUSINESS_FILTER_KEYWORD` 常量修改或关闭（`None`）。
...
- **展开 / 收起缩略节点**：点击缩略节点的**图标**（或工具栏“展开缩略节点”按钮）会把所有缩略节点一次性展开为真实资源节点，业务框、层框等大框自动放大适配；再次点击按钮可收起恢复缩略视图。点击**数字徽标**则在右侧抽屉查看该分组全部资源的明细表格。
