...
Region（区域容器，蓝色虚线框）
  └── 资源类型（ECS、OBS 等服务容器）
        └── 聚合资源节点（普通资源直接挂载；CCE_Deployment 按资源分组保留分组框）
```
...
- 普通资源按 `所属业务 + 所属层 + resource_type + 资源分组 + resource_name 非数字结构` 聚合：只忽略资源名中的数字串，其他字符必须完全一致；CCE_Deployment 保持按原资源分组规则聚合；所有受支持服务类型均使用 `...+N` 摘要节点，同一资源分组的明细全部保留在右侧抽屉中。
- 资源分组为空时归入 `(未设置资源分组)`；不同架构层的同名资源分组不会合并。
...
- 普通资源聚合节点直接挂载在资源类型框内；CCE_Deployment 保留资源分组框。点击 `...+n` 摘要节点或数量徽标后，右侧抽屉展示该分组全部资源明细，并提供逐行“编辑”按钮。
- 容器框文字不缩写、不省略：若文字比框宽，框会自动加宽以完整显示文字。
- 资源节点的名称**完整显示**，不再用 `...` 截断，并按节点宽度主动**折成多行**（长名称如 `deepseek-llm-...` 也会分行展示），不再一行溢出重叠。
- 同一层内的资源类型保持**一行排开**，不再挤压重叠：布局阶段为资源类型框、CCE 资源分组框、层框和业务框计算独立坐标，不同资源类型（如 CCE、ECS）的服务框互不重叠。
- 业务分组默认完全依据 Excel 的 `所属业务` 字段；如需兼容旧场景，可通过 `converter.py` 中的 `BUSINESS_FILTER_KEYWORD` 显式启用关键字过滤（默认 `None`）。
- **全量资源聚合**：普通资源按架构层、`resource_type`、资源分组和资源名非数字结构生成缩略节点（`分组名 + ...+N`），直接置于对应资源类型大框（如 `ECS`）中；CCE_Deployment 按原资源分组规则聚合，保留资源分组框，框名使用 Excel 的 `资源分组` 值。
- 点击缩略节点或其**数字徽标**，右侧抽屉展示该分组全部资源明细；明细表的“编辑”按钮打开节点编辑器，修改后可通过“导出编辑表”保存。
- 点击工具栏“保存布局”可持久化当前节点位置和视角，刷新同一 HTML 后自动恢复；未点击保存时拖拽不会覆盖原始生成布局。
