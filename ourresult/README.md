...
- 资源按 `所属业务 + 所属层 + resource_type + 资源分组 + resource_name 非数字结构` 聚合：只忽略资源名中的数字串，其他字符必须完全一致；所有受支持服务类型均使用 `...+N` 摘要节点，同一资源分组的明细全部保留在右侧抽屉中。
...
- **全量资源聚合**：每个业务内按架构层、`resource_type`、资源分组和资源名非数字结构生成缩略节点（`分组名 + ...+N`），同一 `resource_type` 的所有缩略节点统一置于对应的大框（如 `ECS`）中；大框名称使用 `resource_type`，资源分组框名称使用 Excel 的 `资源分组` 值。
