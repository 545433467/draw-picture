...
- `资源分组` 值为 `CCE_Deployment`（或服务类型标注为 CCE_Deployment）的资源，不再渲染任何单个资源节点，也不做“前 5 个 + 摘要”折叠；每个企业项目只在业务内的“第三层：计算、容器与应用服务层”中显示一个聚合节点：节点使用 `ourresult/picture/CCE_Deployment.png` 图标（若测试电脑等环境存在该图片），图标下方为企业项目名（为空时回退到 `enterprise_project_id`），图标上方带一个独立红色圆圈徽标，圆圈内为红色数字，表示该企业项目下的 CCE_Deployment 总数；点击聚合节点或其数字徽标，侧边栏会列出该企业项目下所有节点的完整信息。其他云资源（ECS、ELB 等）继续直接读取 `ourresult/picture` 中对应的 PNG 图标。
...
- 当 Excel 中存在 `是否核心业务` 列时，只有值为 `是` 的节点会被绘制，其余节点不显示，也不会因为它们被引用而生成外部节点或连线；CCE_Deployment 聚合节点的数量只统计核心部署，某个企业项目下没有核心部署时该聚合节点不生成。若 Excel 中没有这一列，则所有节点照常绘制。
...
- 同一业务内四层架构之间自动生成**固定层级关系**箭头（接入层 → 网络与负载均衡层 → 计算、容器与应用服务层 → 中间件、数据与存储层），该箭头为独立且加粗的深灰色实线箭头，不计入“确认/推断连线”统计；点击任意节点时只高亮该节点的有向上下游链路，固定层级关系边不参与链路展开。
