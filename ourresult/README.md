...
- `资源分组` 值为 `CCE_Deployment`（或服务类型标注为 CCE_Deployment）的资源，不再渲染任何单个资源节点，也不做“前 5 个 + 摘要”折叠；每个企业项目只在业务内的“第三层：计算、容器与应用服务层”中显示一个聚合节点：节点使用 `ourresult/picture/CCE_Deployment.png` 图标（若测试电脑等环境存在该图片），图标下方第一行为企业项目名（为空时回退到 `enterprise_project_id`），第二行为该企业项目下的 CCE_Deployment 总数；点击聚合节点，侧边栏会列出该企业项目下所有节点的完整信息。其他云资源（ECS、ELB 等）继续直接读取 `ourresult/picture` 中对应的 PNG 图标。
